        ...
        <div class="grid grid-cols-12">
            ...
            <div class="col-span-12 md:col-span-4 bg-slate-800/50 p-10 rounded-l-[3rem] border-r border-slate-800 relative overflow-hidden">
                ...
                <div class="relative z-10">
                    ...
                    <p class="text-xs text-slate-400 leading-relaxed mb-8">
                        Lengkapi informasi dasar dokumen RKA sebelum mengisi rincian biaya.
                    </p>

                    <div class="bg-slate-900/80 p-6 rounded-3xl border border-slate-700">
                        <div class="text-[10px] text-slate-500 uppercase font-bold mb-2">Sub Kegiatan Terpilih</div>
                        ...
                        <div class="mt-4 pt-4 border-t border-slate-800 space-y-3">
                            <div class="flex justify-between items-center">
                                <div>
                                    ...
                                    <div class="text-xs font-mono text-emerald-400 font-bold">
                                        ...
                                        @else
                                            <span class="text-rose-400">Belum diset</span>
                                        @endif
                                    </div>
                                </div>
                            ...
                            <div class="pt-2 border-t border-slate-800/50">
                                <div class="flex justify-between text-[10px] text-slate-400">
                                    ...
                                    <span>Rp {{ number_format($totalTerpakai, 0, ',', '.') }}</span>
                                </div>
                                <div class="flex justify-between text-[10px] font-bold mt-1 {{ ($sub->activity->pagu_anggaran - $totalTerpakai) < 0 ? 'text-rose-500' : 'text-slate-300' }}">
                                    <span>Sisa Tersedia:</span>
                                    <span>Rp {{ number_format($sub->activity->pagu_anggaran - $totalTerpakai, 0, ',', '.') }}</span>
                                </div>
                            </div>
                            @endif
                        </div>
                    ...
                    {{-- Opsional: Tampilkan Nama PPTK kecil di sidebar agar user tetap tahu siapa penanggung jawabnya --}}
                    <div class="mt-6 px-2">
                        <div class="text-[9px] text-slate-500 uppercase font-bold">PPTK Penanggung Jawab:</div>
                        <div class="text-[11px] text-slate-300 font-medium italic">{{ $pptkNama ?? '-' }}</div>
                    </div>
                </div>
            </div>
            ...
            <div class="col-span-12 md:col-span-8 p-10 md:p-14">
                <form action="{{ route('rka.store_header') }}" method="POST" class="space-y-6">
                    ...
                    {{-- INPUT HIDDEN: Nama & NIP PPTK otomatis dari controller --}}
                    <input type="hidden" name="nama_pptk" value="{{ $pptkNama }}">
                    <input type="hidden" name="nip_pptk" value="{{ $pptkNip }}">
                    ...
                    <div class="pt-6">
                        <button type="submit" class="w-full py-4 bg-indigo-600 hover:bg-indigo-500 text-white rounded-2xl font-black uppercase tracking-widest text-xs transition-all flex items-center justify-center gap-3">
                            Lanjut ke Rincian <i class="fas fa-arrow-right"></i>
                        </button>
                    </div>
                </form>
            </div>
